...
class Qlock_Matrix:
    ...
    def __init(self):
        self.__led_look_up_table = np.zeros((self.num_letter_vertical * self.num_letter_horizontal * self.leds_per_letter, 2), dtype = int)
        
        
        for i in range(self.num_letter_horizontal):
            for j in range(self.__num_leds_vertical):
                if (i % self.leds_per_letter == 0):
                    self.__led_look_up_table[i * self.__num_leds_vertical + j, 1] = j
                else:
                    self.__led_look_up_table[i * self.__num_leds_vertical + j, 1] = self.__num_leds_vertical - j - 1
    ...
    def enable_letter(self, pos_vertical, pos_horizontal):
        if (pos_vertical >= 0) and (pos_vertical < self.num_letter_vertical) and (pos_horizontal >= 0) and (pos_horizontal < self.num_letter_horizontal):
